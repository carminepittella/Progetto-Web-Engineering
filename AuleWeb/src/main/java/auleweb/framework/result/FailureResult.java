...
public class FailureResult {
    ...
    public void activate(String message, HttpServletRequest request, HttpServletResponse response) {
        try {
            ...
            // limitati a informazioni generiche, non a stringhe di complete di eccezione
            // e.g., potremmo mappare solo la classe dell'eccezione (IOException,
            // SQLException, ecc.) in messaggi come "Errore IO", "Errore database", ecc.
            // WARNING: in a production environment, error messages MUST be limited to
            // generic information, not full exception strings
            // e.g., we may map the exception class only (IOException, SQLException, etc.)
            // to messages like "IO Error", "Database Error", etc.

            // se abbiamo registrato un template per i messaggi di errore, proviamo a usare
            // quello
            ...
            message += ". In addition, the following exception was generated while trying to display the error page: "
                    + ex.getMessage();
            try {
                response.sendError(HttpServletResponse.SC_INTERNAL_SERVER_ERROR, message);
